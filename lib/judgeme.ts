...
export async function getShopReviews(perPage = 4): Promise<JudgeMeAggregated> {
  const fallback: JudgeMeAggregated = { reviews: [], averageRating: 0, totalCount: 0 };

  try {
    const url = `https://judge.me/api/v1/reviews?shop_domain=${SHOP_DOMAIN}&per_page=${perPage}&api_token=${API_TOKEN}`;
